...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $type = filter_input(INPUT_POST, "incidentType", FILTER_VALIDATE_INT);
        $severity = filter_input(INPUT_POST, "optradio", FILTER_VALIDATE_INT);
        ...
        if (empty($errors)) {
            ...
            // 1. Insert into Incident table
            $stmt = $mysqli->prepare("INSERT INTO Incident (description, severity, type_id, date_incident) 
                                    VALUES (?, ?, ?, ?)");
            if (!$stmt) {
                throw new Exception("Prepare 1 failed: " . $mysqli->error);
            }
            
            $stmt->bind_param('siis', $comment, $severity, $type, $currentDateTime);
            if (!$stmt->execute()) {
                throw new Exception("Execute 1 failed: " . $stmt->error);
            }
            $incidentId = $mysqli->insert_id;
            $stmt->close();

            // 2. Insert into IncidentStatus table
            $stmt = $mysqli->prepare("INSERT INTO IncidentStatus (incident_id, status_id, updated_by, updated_at) 
                                    VALUES (?, 1, ?, ?)");
            if (!$stmt) {
                throw new Exception("Prepare 2 failed: " . $mysqli->error);
            }
            
            $stmt->bind_param('iis', $incidentId, $userId, $currentDateTime);
            if (!$stmt->execute()) {
                throw new Exception("Execute 2 failed: " . $stmt->error);
            }
            $incidentStatusId = $mysqli->insert_id;
            $stmt->close();

            // 3. Insert affected assets
            if (!empty($affectedAssets)) {
                $stmt = $mysqli->prepare("INSERT INTO AffectedAsset (incident_status_id, asset_id) 
                                        VALUES (?, ?)");
                if (!$stmt) {
                    throw new Exception("Prepare 3 failed: " . $mysqli->error);
                }
                
                foreach ($affectedAssets as $assetId) {
                    $stmt->bind_param('ii', $incidentStatusId, $assetId);
                    if (!$stmt->execute()) {
                        throw new Exception("Execute 3 failed: " . $stmt->error);
                    }
                }
                $stmt->close();
            }

            // 4. Insert Attachments logic 
        ...
        <div class="content-body">
            ...
            <div class="row page-titles mx-0">
                <div class="col p-md-0">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active"><a href="javascript:void(0)">Create a report</a></li>
                    </ol>
                </div>
            </div>
            <!-- row -->
            ...
            <div class="container-fluid">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">Create a Report</h4>
                            <div class="basic-form">
                                <form action="create-report.php" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <select id="inputType" name="incidentType" class="form-control" required>
                                                <option value="" selected disabled>Choose...</option>
                                                <option value="1">Insider threats</option>
                                                <option value="2">Man-in-the-middle</option>
                                                <option value="3">Password attack</option>
                                                <option value="4">Denial of service</option>
                                                <option value="5">Phishing attacks</option>
                                                <option value="6">Privilege escalation</option>
                                                <option value="7">Ransomware</option>
                                                <option value="8">Unauthorized access attacks</option>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="form-group">
                                            <div class="radio mb-3">
                                                <label>
                                                    <input type="radio" name="optradio" value="1" id="low" required>
                                                    <span class="label gradient-1 ml-2 btn-rounded">Low
                                                        (1)</span></label>
                                            </div>
                                            <div class="radio mb-3">
                                                <label>
                                                    <input type="radio" name="optradio" value="2" id="Medium">
                                                    <span class="label gradient-3 ml-2 btn-rounded">Medium
                                                        (2)</span></label>
                                            </div>
                                            <div class="radio mb-3">
                                                <label>
                                                    <input type="radio" name="optradio" value="3" id="High">
                                                    <span class="label gradient-2 ml-2 btn-rounded">High
                                                        (3)</span></label>
                                            </div>
                                            <div class="radio mb-3">
                                                <label>
                                                    <input type="radio" name="optradio" value="4" id="Critical">
                                                    <span class="label gradient-9 ml-2 btn-rounded">Critical
                                                        (4)</span></label>
                                            </div>
                                        </div>
                                    </div>
