<?php
	...
	$query = "
        SELECT it.incident_type, COUNT(*) as count
		FROM Incident i
		JOIN IncidentType it ON it.incident_type_id = i.type_id
		GROUP BY it.incident_type
    ";

	$resultReportType = $mysqli->query($query);
    while ($row = $resultReportType->fetch_assoc()) {
        $dataReportType['labels'][] = $row['incident_type'];
        $dataReportType['counts'][] = $row['count'];
    }
    ...
    $query = "
		...
		FROM Incident i
		JOIN Severity se ON se.severity_id = i.severity
		GROUP BY se.severity
    ";
    
    $resultReportSeverity = $mysqli->query($query);
    while ($row = $resultReportSeverity->fetch_assoc()) {
        $dataReportSeverity['labels'][] = $row['severity'];
        $dataReportSeverity['counts'][] = $row['count'];
    }
...
***********************************-->
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-6">
                        <div class="card">
                            <div class="card-body" style="height: 500px;">
                                <h4 class="card-title text-center">Incident by type</h4>
                    ...
                    <div class="col-lg-6">
                        <div class="card">
                            <div class="card-body" style="height: 500px;">
                                <h4 class="card-title text-center">Incident by severity</h4>
                                <div class="d-flex justify-content-center align-items-center"
                                    style="height: calc(100% - 40px);">
    ...
    ***********************************-->
        <div class="footer">
            <div class="copyright">
                <p>Copyright &copy; Designed & Developed by <a href="https://themeforest.net/
...
***********************************-->
    ...
    const userChart = new Chart(userChartDOM, {
        type: 'bar',
        data: {
            labels: <?= json_encode($dataReportType['labels']) ?>,
            datasets: [{
                label: 'Incidents by Type',
                data: <?= json_encode($dataReportType['counts']) ?>,
    ...
    const statusDonutChart = new Chart(statusDonutChartDOM, {
        type: 'doughnut',
        data: {
            labels: <?= json_encode($dataReportSeverity['labels']) ?>,
            datasets: [{
                label: 'Incidents by Severity',
                data: <?= json_encode($dataReportSeverity['counts']) ?>,
                backgroundColor: [
